...
// userContextKey is unexported so the authenticated user can only be set/read
// through SetUser / GetUser, never via raw c.Get / c.Set elsewhere.
type userContextKey struct{}

var ctxUser = userContextKey{}
...
func GetUser(c echo.Context) (*user.User, error) {
	if u, ok := c.Get(keyString(ctxUser)).(*user.User); ok && u != nil {
		return u, nil
	}
	return nil, echo.NewHTTPError(http.StatusUnauthorized, "user not found in context")
}

// echo.Context.Set requires a string key; keyString derives a stable private
// key so the value cannot be accessed with a guessable literal.
func keyString(userContextKey) string { return "admin:auth:user" }
