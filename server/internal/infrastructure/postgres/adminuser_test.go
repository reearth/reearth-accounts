...
// Reproduces the REL-01 race directly against Postgres: two approved
// system_admins demoted concurrently must not both succeed, since the
// existence check and the save are now serialized by a pg_advisory_xact_lock
// inside a single transaction (see SaveGuardingLastSystemAdmin).
