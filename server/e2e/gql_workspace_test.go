...
func baseSeederWorkspace(ctx context.Context, r *repo.Container) error {
	...
	// Create permittables for users with workspace roles
	...
	// uId is owner of wId and wId2
	p1, err := permittable.New().
		NewID().
		UserID(uId).
		Build()
	if err != nil {
	...
	// uId3 is reader of wId2
	p3, err := permittable.New().
		NewID().
		UserID(uId3).
		Build()
	if err != nil {
	...
	p3.UpdateWorkspaceRole(wId2, readerRole.ID())
