	...
	tests := []struct {
	...
	}{
		{
			...
			ExpectedWorkspace: New().
				...
				Name("nnn").
				Alias("nnn").
				Members(map[user.ID]Member{uid: {Role: role.RoleOwner, Disabled: false, InvitedBy: uid}}).
				Personal(true).
				MustBuild(),
			Err: nil,
		},
		...
		},
		{
			...
			ExpectedWorkspace: New().
				...
				Name("nnn").
				Alias("nnn").
				Members(map[user.ID]Member{user.NewID(): {Role: role.RoleOwner, Disabled: false, InvitedBy: user.NewID()}}).
				Personal(true).
				MustBuild(),
			Err: nil,
		},
