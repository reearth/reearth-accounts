...
func TestWorkspaceRepo_FindByIDs(t *testing.T) {
	ctx := context.Background()

	t.Run("successfully find workspaces by ids", func(t *testing.T) {
		httpmock.Activate()
		defer httpmock.DeactivateAndReset()
						...
						"findByIDs": [
							{
								...
								"metadata": {
									...
									"location": "Tokyo",
									"billingEmail": "a-billing@example.com",
									"photoURL": "https://a.example.com/photo.jpg"
								},
								"members": [
									{
										"userId": "01j9x0yy00000000000000000a",
							...
							},
							{
								...
								"metadata": {
									...
									"location": "Osaka",
									"billingEmail": "b-billing@example.com",
									"photoURL": "https://b.example.com/photo.jpg"
								},
								"members": [
									{
										"userId": "01j9x0yy00000000000000000a",
		...
		ids := []string{
			"01j9x0yy00000000000000001a",
			"01j9x0yy00000000000000002a",
		}
		got, err := client.WorkspaceRepo.FindByIDs(ctx, ids)

		assert.NoError(t, err)
		assert.NotNil(t, got)
		assert.Len(t, got, 2)

		assert.Equal(t, "Workspace A", got[0].Name())
		assert.Equal(t, "a", got[0].Alias())
		assert.False(t, got[0].IsPersonal())

		assert.Equal(t, "Workspace B", got[1].Name())
		assert.Equal(t, "b", got[1].Alias())
		assert.True(t, got[1].IsPersonal())
	})
	...
	t.Run("empty workspace list", func(t *testing.T) {
		...
		ids := []string{
			"01j9x0yy00000000000000003a",
			"01j9x0yy00000000000000004a",
		}
		got, err := client.WorkspaceRepo.FindByIDs(ctx, ids)
...
func TestWorkspaceRepo_UpdateWorkspace(t *testing.T) {
	...
	t.Run("successfully update workspace with all fields", func(t *testing.T) {
		httpmock.Activate()
		defer httpmock.DeactivateAndReset()
		...
		httpmock.RegisterResponder("POST", "https://accounts.example.com/api/graphql",
			func(req *http.Request) (*http.Response, error) {
				bodyBytes, _ := io.ReadAll(req.Body)
				fmt.Printf("\n--- UpdateWorkspace All Fields GraphQL Request ---\n%s\n", string(bodyBytes))
				req.Body = io.NopCloser(bytes.NewBuffer(bodyBytes))
				return httpmock.NewStringResponse(http.StatusOK, `{
					"data": {
						"updateWorkspace": {
		...
		assert.NotNil(t, got)
		assert.Equal(t, "Updated Workspace", got.Name())
		assert.Equal(t, "new-alias", got.Alias())
	})
}
...
func TestWorkspaceRepo_FindByAlias(t *testing.T) {
	...
	t.Run("successfully find workspace by alias", func(t *testing.T) {
		...
		got, err := client.WorkspaceRepo.FindByAlias(ctx, "test-workspace")
	...
	t.Run("workspace not found by alias (alias is available)", func(t *testing.T) {
		httpmock.Activate()
		defer httpmock.DeactivateAndReset()
		...
		got, err := client.WorkspaceRepo.FindByAlias(ctx, "nonexistent-alias")
